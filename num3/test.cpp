...
void testCompleteBinaryTree() {
    ...
    ofstream text("cbt_test.txt");
    t1.saveToFile(text);
    text.close();

    CompleteBinaryTree t2;
    ifstream textIn("cbt_test.txt");
    t2.loadFromFile(textIn);
    textIn.close();
    ...
    ofstream bin("cbt_test.bin", ios::binary);
    t1.saveToBinaryFile(bin);
    bin.close();

    CompleteBinaryTree t3;
    ifstream binIn("cbt_test.bin", ios::binary);
    t3.loadFromBinaryFile(binIn);
    binIn.close();
