...
type TwoList struct {
    head *node.TwoListNode
    tail *node.TwoListNode
}
...
func (tl *TwoList) AddToHead(val string) {
    newNode := &node.TwoListNode{Data: val}
    if tl.head == nil {
        tl.head = newNode
        tl.tail = newNode
    } else {
...
func (tl *TwoList) AddAfterValue(targetVal, newVal string) error {
    ...
    newNode := &node.TwoListNode{Data: newVal}
...
func (tl *TwoList) findByVal(val string) *node.TwoListNode {
    curr := tl.head
    for curr != nil {
        if curr.Data == val {
            return curr
