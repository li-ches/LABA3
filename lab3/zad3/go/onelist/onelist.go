...
type OneList struct {
	head *node.OneListNode
	tail *node.OneListNode
}
...
func (ol *OneList) AddToHead(val string) {
	newNode := &node.OneListNode{Data: val}
...
func (ol *OneList) AddAfterValue(targetVal, newVal string) error {
	...
	newNode := &node.OneListNode{Data: newVal}
...
func (ol *OneList) findByVal(val string) *node.OneListNode {
	curr := ol.head
	for curr != nil {
		if curr.Data == val {
			return curr
