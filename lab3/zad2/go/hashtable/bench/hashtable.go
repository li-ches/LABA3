...
func NewCuckooHashTable(capacity int) *CuckooHashTable {
	if capacity <= 0 {
		capacity = DefaultInitialCapHash
	}
	...
	table := &CuckooHashTable{
		...
		table1:        make([]CKNode, capacity),
		table2:        make([]CKNode, capacity),
	}
...
func (c *CuckooHashTable) hash1(key string) int {
	hash := 0
	for _, ch := range key {
		hash = (hash*FirstHash + int(ch)) % c.capacity
	}
	return hash
}
...
func (c *CuckooHashTable) hash2(key string) int {
	hash := 0
	for _, ch := range key {
		hash = (hash*SecondHash + int(ch)) % c.capacity
	}
	return hash
}
...
func (c *CuckooHashTable) rehash(newCapacity int) bool {
	...
	c.table1 = make([]CKNode, newCapacity)
	c.table2 = make([]CKNode, newCapacity)
...
func (c *CuckooHashTable) Print() {
	...
	fmt.Printf("Размер: %d/%d (нагрузка: %.2f%%)\n", c.size, c.capacity, float64(c.size)/float64(c.capacity)*float64(Proc))
...
func (d *DoubleHashTable) hash1(key string) int {
	hash := 0
	for _, ch := range key {
		hash = (hash*FirstHash + int(ch)) % d.capacity
	}
	return hash
}
...
func (d *DoubleHashTable) hash2(key string) int {
	hash := 0
	for _, ch := range key {
		hash = (hash*SecondHash + int(ch)) % d.capacity
	}
	return hash
}
...
func (d *DoubleHashTable) rehash(newCapacity int) bool {
	...
	d.table = make([]DHNode, newCapacity)
...
func (d *DoubleHashTable) Insert(key string, data string) bool {
	...
	if loadFactor > LoadFactorDouble {
...
func (d *DoubleHashTable) Print() {
	...
	fmt.Printf("Размер: %d/%d (нагрузка: %.2f%%)\n", d.size, d.capacity, float64(d.size)/float64(d.capacity)*float64(Proc))
